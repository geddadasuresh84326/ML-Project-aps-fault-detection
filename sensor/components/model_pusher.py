...
class ModelPusher:
    ...
    def initiate_model_pusher(self)->ModelPusherArtifact:
        try:
            ...
            # load object
            logging.info(f"Model pusher initiated")
            logging.info(f"loading transformer,model and target_encoder object")
            transformer = load_object(file_path=self.data_transformation_artifact.transform_object_path)
            model = load_object(file_path=self.model_trainer_artifact.model_path)
            target_encoder = load_object(file_path=self.data_transformation_artifact.target_encoder_path)

            # model pusher dir
            logging.info(f"Saving objects into model pusher dir")
            save_object(file_path = self.model_pusher_config.pusher_transformer_path,obj=transformer)
            save_object(file_path = self.model_pusher_config.pusher_model_path,obj=model)
            save_object(file_path = self.model_pusher_config.pusher_target_encoder_path,obj=target_encoder)

            # save model dir
            logging.info(f"Saving objects into save model dir")
